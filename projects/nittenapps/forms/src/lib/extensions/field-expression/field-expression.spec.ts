...
import { createBuilder } from '@nittenapps/forms/testing';
import { of, Subject } from 'rxjs';
import { StackFieldConfig, StackFieldConfigCache } from '../../types';

function buildField({ model, options, ...field }: StackFieldConfigCache): StackFieldConfigCache {
  ...
  describe('expressions', () => {
    ...
    describe('field disabled state', () => {
      ...
      it('should throw error when assign to an undefined prop', () => {
        ...
        expect(build).toThrow(
          /\[StackForms Error\] \[Expression "nested.prop"\] (Cannot set property 'prop' of undefined|Cannot set properties of undefined \(setting 'prop'\))/i
        );
      });
    });
    ...
    describe('model changes', () => {
      it('should emit formControl value changes', () => {
        const { fieldGroup: fields, options, model } = buildField({
          fieldGroup: [
            {
              key: 'text',
              type: 'input',
    ...
    it('should check expression when detecting new field changes', () => {
      ...
      expect(fields?.[1].hide).toBeFalse();
      expect(fields?.[2].hide).toBeFalse();
    });

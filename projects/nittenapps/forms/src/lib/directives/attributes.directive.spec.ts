...
import { createComponent, ɵCustomEvent } from '@nittenapps/forms/testing';
import { StackFieldConfig } from '../types';

const renderComponent = (field: StackFieldConfig, { template }: { template?: string } = {}) => {
  return createComponent<{ field: StackFieldConfig }>({
    template: template || '<input type="text" [nasFormsAttributes]="field">',
    inputs: { field },
  });
};

describe('StackFormsAttributes Component', () => {
  describe('props attributes', () => {
    ...
    it('should ignore handle attr if already set', () => {
      const { query } = renderComponent(
        {},
        {
          template: `<input type="text" [attr.disabled]="'disabled'" [nasFormsAttributes]="field">`,
        }
      );
  ...
  describe('focus the element', () => {
    ...
    it(`should set id to the first element only when mutliple nasFormsAttributes is present`, () => {
      const { queryAll } = renderComponent(
        { id: 'title-id' },
        {
          template: `
            <input type="text" [nasFormsAttributes]="field">
            <input type="text" [nasFormsAttributes]="field">
          `,
        }
      );
      const inputs = queryAll('input') as any;
    ...
    it(`should not set field id when id input is present`, () => {
      const { query } = renderComponent(
        ...
        {
          template: `
            <input [id]="'foo'" type="text" [nasFormsAttributes]="field">
          `,
        }
      );
    ...
    it(`should focus the first element when mutliple nasFormsAttributes is present`, fakeAsync(() => {
